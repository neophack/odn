 ...
 */
int fw_printenv (char *envname)
{
	char *env, *nxt;
	int rc = 0;

	if (!strcmp(envname, "all")) {		/* Print all env variables  */
		for (env = environment.data; *env; env = nxt + 1) {
			for (nxt = env; *nxt; ++nxt) {
				if (nxt >= &environment.data[ENV_SIZE]) {
					fprintf (stderr, "## Error: "
 ...
 *
 */
int fw_setenv (char *name, char *setenv_val)
{
	int len;
	char *env, *nxt;
	char *oldval = NULL;
	 ...
	 * 解读：从env数据的开始往后查找要设置的那个环境变量名字,如果找到了,env指向
	 * 那个变量的开始，nxt指向这个环境变量的'\0'处，而oldval指向这个变量的值
	 * 比如说，bootdelay=2\0这个变量
	 *         ^         ^ ^   
	 *		   |	     | |
	 *        env      val nxt
	 * env的存放格式是：‘bootdelay=2\0hellworld=3\0...endenv=nihao\0\0
	 * 也就说，相邻环境变量之间用\0隔开，环境变量收尾用\0\0
	 */
	...
	/* if this env is exist, then delete this env.*/
	if (oldval) {
		/*
		...
		if (*++nxt == '\0') {/* if get the endline of env, revalue env = \0*/
			*env = '\0';
		} else {
			for (;;) {
				*env = *nxt++;
...
/*
 *fw_saveenv : saveenv into flash.
 *
 * return value: 
 *  -1 : write flash failed.
 *  0  : write flash successd!
 * */
int fw_saveenv(void)
{
	/*
