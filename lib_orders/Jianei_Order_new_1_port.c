...
void Jianei_Order_new_1_port(unsigned char *src, unsigned char data_source, int sockfd)
{
	...
	if(data_source == DATA_SOURCE_MOBILE || data_source == DATA_SOURCE_NET)
	{
		if(0xa2 != src[21] && 0x9a != src[21]){
			...
			if(!myStringcmp(&boardcfg.box_id[0],&src[WEBMASTER_DATA_UUID_INDEX], WEBMASTER_DATA_UUID_LENGTH)){
				orderoperations_failure_and_reply_to_mobile(CMD_NEW_1_PORTS, 0x01);
				return ;
			}
			...
			Jianei_Order.zkb_rep0d[4] = CMD_NEW_1_PORTS;
			Jianei_Order.zkb_rep0d[5] = 0x42;
			ofs = 6;
			...
			if(!module_order_tasks_to_sim3u1xx_board()){
				orderoperations_failure_and_reply_to_mobile(CMD_NEW_1_PORTS, 0x03);
				return ;
			}
	...
	}else if(data_source == DATA_SOURCE_UART)
	{
		...
		if(0x03 == src[5]){
            ...
            if(myStringcmp(&Jianei_Order.eid_port1[0], &src[7], 3)){
				...
				ofs += 4;
				memcpy(&Jianei_Order.eid_port1[4], &Jianei_Order.eid_port1[4], eidtype);
				ofs += 32;
		...
		}else if(0x0a == src[5])
		{
			...
			if(0x0a == Jianei_Order.eid_port1[40] ){
				/* 工单完成 */
				orderoperations_success_and_reply_to_mobile(NULL, CMD_NEW_1_PORTS, 0x00);
				...
				orderoperations_failure_and_reply_to_mobile(CMD_NEW_1_PORTS, 0x07);
			}
